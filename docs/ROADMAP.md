...
| P5 | Aprovar o rascunho do CCAE v1.2 com o orientador (`docs/CCAE-v1.2.md`) | Clarifica a §7.1, ramo (b), ligando F1 à dose em vez do gatilho — fecha B-5 e B-6 sem alterar limiar nem gabarito. Antes de submeter, resolver a única cláusula em aberto: se F1 lê `NC` antes ou depois do arredondamento de apresentação | pendente — decisão do autor, depois aval do orientador e tag `criterios-v1.2` |
